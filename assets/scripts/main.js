...
Config.Categories.forEach((cat, index) => {
  $(".categories .buttons").append(
    `<p data-category="${index}" class="${cat.default ? "active" : ""}">${cat.label}</p>`,
  );
...
Config.Socials.forEach((social, index) => {
  $(".categories .socialmedia").append(
    `<div class="box" data-id="${social.name}" data-link="${social.link}"><img class="icon" src="${social.icon}"><div class="info"><p class="title">${social.label}</p><p class="description">${social.description}</p></div>`,
  );
});
...
$(".staff .next").on("click", function () {
  if (currentPage < Config.Staff.length - 2) {
    ...
    $(".staff .innercards").css(
      "transform",
      `translate3d(calc(-${currentPage * 50}% - ${(currentPage + 1) * 0.5}vw), 0, 0)`,
    );
  }
});
